...
void main() {
  group('NotificationCenterController', () {
    ...
    test('markAllSeen quiets the bell once completions are acknowledged', () {
      ...
      expect(controller.aggregateStatus, NotificationBellAggregateStatus.none);
    });

    test(
      'markAllSeen does NOT quiet the bell while items are still running',
      () {
    ...
    test('failed items take priority over running in aggregate', () {
      ...
      expect(controller.aggregateStatus, NotificationBellAggregateStatus.error);
      expect(controller.homogeneousCount, isNull);
    });

    test(
      'running items keep the bell running even when a sibling completes',
      () {
        final controller = NotificationCenterController();
