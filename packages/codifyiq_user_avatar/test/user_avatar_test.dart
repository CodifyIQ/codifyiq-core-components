...
void main() {
  group('UserAvatar.initialsFor', () {
    ...
    test('three-or-more-word name → first+last initials', () {
      expect(UserAvatar.initialsFor(displayName: 'Mary Jackson Smith'), 'MS');
    });
    ...
    test('parenthetical qualifier is stripped before deriving initials', () {
      expect(
        UserAvatar.initialsFor(displayName: 'Java Joe (Contractor)'),
        'JJ',
      );
      // Qualifier on a single-word name → first two of the remaining word.
      expect(UserAvatar.initialsFor(displayName: 'Linus (Admin)'), 'LI');
      // Mid-name qualifier does not fuse the surrounding words.
    ...
    test('name that is entirely parenthetical falls through to email', () {
      ...
      // …and to empty when there is no email either.
      expect(UserAvatar.initialsFor(displayName: '(Contractor)'), '');
    });
    ...
    test('email used only when displayName is null or blank', () {
      expect(UserAvatar.initialsFor(email: 'kathleen.booth@example.com'), 'KA');
      expect(
  ...
  group('UserAvatar widget', () {
    ...
    testWidgets('imageProvider takes precedence and bypasses the builder', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        pump(
          UserAvatar(
            ...
            displayName: 'Ada Lovelace',
            imageProvider: const AssetImage('__never_loads__'),
            imageProviderBuilder: (url, headers) {
              builderCalled = true;
              return const AssetImage('__builder__');
            },
          ),
        ),
      );
      // The explicit provider wins; the photo path renders an Image, not
      // initials, and the URL builder is never consulted.
      expect(builderCalled, isFalse);
      expect(find.byType(Image), findsOneWidget);
      expect(find.text('AL'), findsNothing);
    });

    testWidgets('imageProvider renders the photo path without any photoUrl', (
      tester,
    ) async {
      await tester.pumpWidget(
        pump(
          const UserAvatar(
            displayName: 'Ada Lovelace',
            imageProvider: AssetImage('__never_loads__'),
          ),
        ),
      );
      expect(find.byType(Image), findsOneWidget);
    ...
    testWidgets('clearing imageProvider rebuilds and falls back to initials', (
      tester,
    ) async {
      Widget build(ImageProvider? provider) => pump(
        UserAvatar(displayName: 'Ada Lovelace', imageProvider: provider),
      );

      await tester.pumpWidget(build(const AssetImage('__never_loads__')));
      expect(find.byType(Image), findsOneWidget);
      expect(find.text('AL'), findsNothing);

      // Recycling the element with a null provider (e.g. a list row reused for
      // a photo-less user) must rebuild the image and reveal the initials.
